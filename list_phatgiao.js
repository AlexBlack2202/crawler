...
function crawlerPage(pageInfo){
    ...
    c.queue([{
        ...
        'callback':function(error,result,$){
            ...
            var totalStory = $('#content table tbody tr').length;

            $('#content table tbody tr').each(function(index,div){

                link = $(div).find('a').eq(0).attr('href');
                name = $(div).find('a').eq(0).text();
                hot = 18;
                status = 'Full';
                trData = {
...
function run(current_page){
    try {
        ...
        var row = {
            link:'http://www.maxreading.com/sach-hay/truyen-co-phat-giao',
            //link:'http://truyenfull.vn/danh-sach/truyen-hot/',
            category_name: 'Truyện Phật Giáo',
            category_slug: 'phat-giao',
            id:32,

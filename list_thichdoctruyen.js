...
function crawlerPage(pageInfo){
    ...
    if(pageInfo.current_page!=1){
        pageInfo.link =pageInfo.link+'/page'+(pageInfo.current_page);
    }
    ...
    jsdom.env({
        ...
        src: [jquery],
            ...
            var totalStory = $('table.tabtruyen tbody tr').length;

            $('table.tabtruyen tbody tr').each(function(index,div){
                if(index<2 || index > totalStory-3){
                    return;
                }

                link = $(div).find('td').eq(1).find('a').attr('href');
                name = $(div).find('.tabtruyen-name').text();
                hot = pageInfo.hot;
                status = 'Đang ra';
                author = $(div).find('td').eq(2).text();
                author  = author.trim('\n');
                author  = author.trim('\t');
                trData = {
...
function run(current_page){
    try {
        ...
        var row = {
            link:'http://thichdoctruyen.com/the-loai/ngon-tinh',
            //link:'http://truyenfull.vn/danh-sach/truyen-hot/',
            category_name: 'Truyện Ngôn Tình',
            category_slug: 'ngon-tinh',
            id:2,
            hot:80,
            site:1,
